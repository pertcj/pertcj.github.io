...
def extract_author_info(config):
    ...
    if "author" in config:
        ...
        if author.get("employer"):
            author_info["summary"] = (
                f"Currently employed at {author.get('employer')}"
            )
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add portfolio
    cv_json["portfolio"] = parse_portfolio(
        os.path.join(repo_root, "_portfolio")
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert markdown CV to JSON format"
    )
    parser.add_argument(
        "--input", "-i", required=True, help="Input markdown CV file"
    )
    parser.add_argument(
