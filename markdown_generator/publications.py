...
def create_md(lines: list, layout: list):
    for item in lines:
        ...
        html_filename = (
            str(item[layout.index("pub_date")])
            + "-"
            + item[layout.index("url_slug")]
        )
        ...
        md_filename = os.path.join(
            "../_publications/", os.path.basename(md_filename)
        )
...
def read(filename: str) -> tuple[list, list]:
    ...
    if len(lines) <= 1:
        print(
            f"Not enough lines in the file to process, found {len(lines)}",
            file=sys.stderr,
        )
        sys.exit(EXIT_ERROR)
    ...
    if HEADER_LEGACY == lines[0]:
        ...
        print(lines[0])
        print(
            "The header of the file does not match the expected format",
            file=sys.stderr,
        )
        sys.exit(EXIT_ERROR)
    lines = lines[1:]
